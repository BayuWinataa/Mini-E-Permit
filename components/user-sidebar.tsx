...
import { LayoutDashboard, FilePlus, X, Menu, FileText } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';

const navItems = [
	{ label: 'Dashboard', href: '/user/dashboard', icon: LayoutDashboard },
	{ label: 'Pengajuan Izin', href: '/user/pengajuan', icon: FilePlus },
];
...
export function UserSidebar({ userName }: Props) {
	...
	return (
		<>
			...
			<aside className={cn('fixed inset-y-0 right-0 z-50 flex w-64 flex-col bg-sidebar shadow-xl transition-transform duration-200 lg:hidden', mobileOpen ? 'translate-x-0' : 'translate-x-full')}>
				...
				<NavBody pathname={pathname} userName={userName} onNavClick={() => setMobileOpen(false)} />
			</aside>
		</>
	);
}
